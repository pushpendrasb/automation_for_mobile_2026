...
| **Appraisals History** (My / All lists) | `HistoryViewController` | `HistoryPage.ts` |
...
Specs live under `tests/history/` (separate from create-appraisal flows).

- `npm run test:ios:history` — all history specs (`tests/history/**/*.test.ts`)
- `npm run test:ios:history:screen` — `AP-HI-P01` only (`history.screen.test.ts`)
- `npm run test:android:appraisal` — all create-appraisal specs on Android
- `npm run test:android:appraisal:success` — happy-path create appraisal (`createAppraisal.flow.test.ts`)
- `npm run test:android:appraisal:failure` — duplicate-registration failure (`createAppraisal.negative.test.ts`)
- `npm run test:android:history` — all history specs on Android
- `npm run test:android:history:screen` — `AP-HI-P01` only on Android

Requires a signed-in account with at least one appraisal visible on **My Appraisals** and/or **All Appraisals**.
